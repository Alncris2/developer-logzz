...
if (isset($_POST) AND (!(empty($_POST['email'])) OR !(empty($_POST['password'])))) {
    ...
    if ($user_password != $password){
        header('Location: ' . SERVER_URI . '/index-no-cript?erro');
        exit;
    }
        ...
        while($row = $get_user_infos->fetch()) {
            ...
            if($row['active'] != 1){
                header('Location: ' . SERVER_URI . '/index-no-cript?accesso-negado');
                exit;
            }
      ...
      <div class="row no-gutters">
          <div class="col-xl-12">
              <div class="auth-form">
                  ...
                  <form action="<?php echo SERVER_URI; ?>/index-no-cript" method="POST">
