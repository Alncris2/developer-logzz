...
if (isset($_POST) AND (!(empty($_POST['email'])) OR !(empty($_POST['password']))  OR !(empty($_POST['email-user'])))) {
    $email = addslashes($_POST['email']);
    $email_user = addslashes($_POST['email-user']); 
    ...
    if ($user_password != $password){
        header('Location: ' . SERVER_URI . '/login-admin-user?erro');
        exit;
    }
    
    $get_user_admin_infos = $conn->prepare('SELECT * FROM subscriptions INNER JOIN users ON users.user__id = subscriptions.user__id WHERE users.user__id = :user__id');
    $get_user_admin_infos->execute(array('user__id' => $user__id));

    if($row_admin = $get_user_admin_infos->fetch()) {
        # Verifica se o perfil do usuário está ativo
        if($row_admin['active'] != 1){
            header('Location: ' . SERVER_URI . '/login-admin-user?accesso-negado');
            exit;
        }

        if($row_admin['user_plan'] != 5 ||  $row_admin['super_admin'] != true ){ 
            header('Location: ' . SERVER_URI . '/login-admin-user?no-admin');
            exit;
        }
    }

    $get_user_infos = $conn->prepare('SELECT * FROM subscriptions INNER JOIN users ON users.user__id = subscriptions.user__id WHERE users.email = :email');
    $get_user_infos->execute(array('email' => $email_user)); 
         
    while($row = $get_user_infos->fetch()) { 

        if($row['user_plan'] == 5){
            header('Location: ' . SERVER_URI . '/login-admin-user?user-admin');
            exit;
        }
    ...
    header('Location: ' . SERVER_URI . '/login-admin-user?erro-user');
    exit;
}
...
<div class="col-md-6">
    ...
    } else if (isset($_GET['no-admin'])) {
        echo '<div class="alert alert-danger alert-dismissible fade show">
        <svg viewBox="0 0 24 24" width="24 " height="24" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round" class="mr-2"><polygon points="7.86 2 16.14 2 22 7.86 22 16.14 16.14 22 7.86 22 2 16.14 2 7.86 7.86 2"></polygon><line x1="15" y1="9" x2="9" y2="15"></line><line x1="9" y1="9" x2="15" y2="15"></line></svg>
        <strong>Erro!</strong> Acesso restrito, somente adminstradores autorizados.
        </div>';
    } else if (isset($_GET['erro-user'])) {
        echo '<div class="alert alert-danger alert-dismissible fade show">
        <svg viewBox="0 0 24 24" width="24 " height="24" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round" class="mr-2"><polygon points="7.86 2 16.14 2 22 7.86 22 16.14 16.14 22 7.86 22 2 16.14 2 7.86 7.86 2"></polygon><line x1="15" y1="9" x2="9" y2="15"></line><line x1="9" y1="9" x2="15" y2="15"></line></svg>
        <strong>Erro!</strong> Usuário não encontrado, verifique o email do usuário em questão.
        </div>';
    } else if (isset($_GET['user-admin'])) {
        echo '<div class="alert alert-danger alert-dismissible fade show">
        <svg viewBox="0 0 24 24" width="24 " height="24" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round" class="mr-2"><polygon points="7.86 2 16.14 2 22 7.86 22 16.14 16.14 22 7.86 22 2 16.14 2 7.86 7.86 2"></polygon><line x1="15" y1="9" x2="9" y2="15"></line><line x1="9" y1="9" x2="15" y2="15"></line></svg>
        <strong>Erro!</strong> Não é possível acessar a conta de outro adminstrador.
        </div>';
    } ?>
      ...
      <div class="row no-gutters">
          <div class="col-xl-12">
              <div class="auth-form">
                  ...
                  <form action="<?php echo SERVER_URI; ?>/login-admin-user" method="POST">  
                      ...
                      <div class="form-group">
                          <!-- <label class="mb-1 text-white"><strong>Email</strong></label> -->
                          <input name="email-user" type="email-user" class="form-control" placeholder="Email do Usuário" required value="">
                      </div>  
