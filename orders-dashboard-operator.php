...
$get_frustrated_deliveries = $conn->prepare("SELECT COUNT(order_status) FROM orders o INNER JOIN local_operations_orders lo ON lo.order_id = o.order_id WHERE o.order_status = 4 AND lo.operation_id = (SELECT logistic_operator.local_operation FROM logistic_operator WHERE logistic_operator.user_id = :user_id)");
$get_frustrated_deliveries->execute(array("user_id" => $user__id));

$get_complete_deliveries = $conn->prepare("SELECT COUNT(order_status) FROM orders o INNER JOIN local_operations_orders lo ON lo.order_id = o.order_id WHERE o.order_status = 3 AND lo.operation_id = (SELECT logistic_operator.local_operation FROM logistic_operator WHERE logistic_operator.user_id = :user_id)");
$get_complete_deliveries->execute(array("user_id" => $user__id));
...
$get_commission = $conn->prepare('SELECT * FROM transactions_meta WHERE user__id=:user__id');
$get_commission->execute(array("user__id" => $user__id));
$comission = $get_commission->fetch()[0];
...
if (isset($_GET['filtro']) && $_GET['filtro'] == "ativo") {
    ...
    } else if (!(empty($_GET['status'])) && $_GET['status'] != 11) {
        # Filtra por STATUS
        $filter_by_status = addslashes($_GET['status']);
        if ($filter_by_status == 7) {
            $filter_by_status = 0;
                        ...
                        <div class="card-body card-dashboard">
                            <div class="d-flex justify-content-between">
                                ...
                                <div>
                                    ...
                                    <span class="fs-20 font-w700 d-block"><small>R$ </small><?php echo number_format($comission, 2, ',', '.'); ?>
                                    </span>
                                </div>
                            </div>
                        </div>
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-lg-12">
                                            ...
                                            <select id="select-filter-status-id" class="d-block default-select">
                                                ...
                                                <option <?php if (@$_GET['status'] == '7') {
                                                            echo "selected";
                                                        } ?> value="7">Agendada</option>
                                                <option <?php if (@$_GET['status'] == '2') {
                                                            echo "selected";
                                                        } ?> value="2">Atrasada</option>
                                                <option <?php if (@$_GET['status'] == '5') {
                                                            echo "selected";
                                                        } ?> value="5">Cancelada</option>
                                                <option <?php if (@$_GET['status'] == '4') {
                                                            echo "selected";
                                                        } ?> value="4">Frustrada</option>
                                                <option <?php if (@$_GET['status'] == '3') {
                                                            echo "selected";
                                                        } ?> value="3">Completa</option>
                                                <option <?php if (@$_GET['status'] == '1') {
                                                            echo "selected";
                                                        } ?> value="1">Reagendada</option>
                                            </select>
