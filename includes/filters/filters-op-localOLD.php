...
<?php
    ...
    if($_SESSION['UserPlan'] == 5){
        ...
        */
        ...
        $create_temp_table = $conn->prepare("CREATE TEMPORARY TABLE orders_all SELECT o.* FROM orders AS o $logistic_or_operator WHERE (o.order_status = :status AND order_delivery_date BETWEEN :date_init AND :date_end) AND platform IS null $per_product $per_user AND o.order_number NOT LIKE 'AFI%' ");
        ...
        if(isset($_GET['status']) && !empty($_GET['status'])){
            ...
            $create_temp_table = $conn->prepare("CREATE TEMPORARY TABLE orders_six SELECT o.* FROM orders_all AS o $logistic_or_operator WHERE (o.order_status = 6 AND o.order_delivery_date BETWEEN :date_init AND :date_end) AND platform IS null $per_product $per_user");
            $create_temp_table->execute(array('date_init' => $date_init, 'date_end' => $date_end));
        }else{
            ...
            $create_temp_table = $conn->prepare("CREATE TEMPORARY TABLE orders_six SELECT o.* FROM orders AS o $logistic_or_operator WHERE (o.order_status = 6 AND o.order_delivery_date BETWEEN :date_init AND :date_end) AND platform IS null $per_product $per_user");
            $create_temp_table->execute(array('date_init' => $date_init, 'date_end' => $date_end));
        }
        ...
        if(isset($_GET['status']) && !empty($_GET['status'])){
            ...
            $create_temp_table = $conn->prepare("CREATE TEMPORARY TABLE orders_six SELECT o.* FROM orders_all AS o $per_local_operator WHERE user__id = :user__id AND (order_status = 6 AND order_delivery_date BETWEEN :date_init AND :date_end) AND platform IS null $per_product");
            $create_temp_table->execute(array('user__id' => $user__id, 'date_init' => $date_init, 'date_end' => $date_end));
        }else{
            ...
            $create_temp_table = $conn->prepare("CREATE TEMPORARY TABLE orders_six SELECT o.* FROM orders AS o $per_local_operator WHERE user__id = :user__id AND (order_status = 6 AND order_delivery_date BETWEEN :date_init AND :date_end) AND platform IS null $per_product");
            $create_temp_table->execute(array('user__id' => $user__id, 'date_init' => $date_init, 'date_end' => $date_end));
        }
    ...
    $get_money_payment = $conn->prepare('SELECT A.quant FROM (SELECT COUNT(order_payment_method) AS quant, order_delivery_date, order_payment_method AS method FROM orders_all WHERE order_payment_method = "money") as A');
    ...
    $get_credit_payment = $conn->prepare('SELECT A.quant FROM (SELECT COUNT(order_payment_method) AS quant, order_delivery_date, order_payment_method AS method FROM orders_all WHERE order_payment_method = "credit") as A');
    ...
    $get_debit_payment = $conn->prepare('SELECT A.quant FROM (SELECT COUNT(order_payment_method) AS quant, order_delivery_date, order_payment_method AS method FROM orders_all WHERE order_payment_method = "debit") as A');
    ...
    $get_pix_payment = $conn->prepare('SELECT A.quant FROM (SELECT COUNT(order_payment_method) AS quant, order_delivery_date, order_payment_method AS method FROM orders_all WHERE order_payment_method = "pix") as A');
    ...
    $faturamento = $conn->prepare('SELECT A.total FROM (SELECT SUM(order_final_price) AS total, order_delivery_date FROM orders_all AS o WHERE o.order_number NOT LIKE "AFI%") as A INNER JOIN sales AS s');
    $faturamento->execute();
   
    $total_witch_producer = $faturamento->fetch()['total'];

    # PEGAR TODAL FATURADO EM VENDAS COMO AFILIADO
    $get_total_sales_afi = $conn->prepare("SELECT o.order_number FROM orders_all AS o INNER JOIN sales AS s ON o.sale_id = s.sale_id AND o.order_number LIKE 'AFI%'");
    ...
    $vendas = $conn->prepare('SELECT COUNT(*), order_delivery_date FROM orders_all WHERE order_number NOT LIKE "AFI%" ');
    $vendas->execute();
    ...
    */
    ...
    $get_products_sum = $conn->prepare("SELECT SUM(sale_quantity) AS sale_quantity FROM orders_all AS o INNER JOIN sales AS s ON s.sale_id = o.sale_id WHERE product_name = :product_name");
    $get_products_id = $conn->prepare("SELECT o.product_id FROM orders_all AS o INNER JOIN sales AS s ON s.sale_id = o.sale_id AND product_name = :product_name");
    $get_user_id = $conn->prepare('SELECT user__id FROM orders_all WHERE product_id = :product_id');
    $specific_sale = $conn->prepare('SELECT COUNT(*) AS count_all, order_datetime FROM orders_all WHERE product_id = :product_id');
    ...
    $specific_invoice = $conn->prepare('SELECT SUM(o.order_final_price) FROM orders_all as o INNER JOIN sales AS s WHERE o.sale_id = s.sale_id AND o.product_id = :product_id');  
    ...
    if($_SESSION['UserPlan'] == 5){
        foreach($daysOfWeek["Domingo"] as $days){
            if(isset($_GET['status']) && !empty($_GET['status'])){
                $query = $conn->prepare("SELECT COUNT(o.order_id) AS total FROM orders AS o $logistic_or_operator INNER JOIN sales AS s ON o.sale_id = s.sale_id WHERE o.order_date LIKE :order_date AND o.order_status = :status $per_product $per_user");
                $query->execute(['order_date' => $days.'%', 'status' => $per_status]);
            }else{
                $query = $conn->prepare("SELECT COUNT(o.order_id) AS total FROM orders AS o $logistic_or_operator INNER JOIN sales AS s ON o.sale_id = s.sale_id WHERE o.order_date LIKE :order_date AND o.order_status = 0 $per_product $per_user");
        ...
        foreach($daysOfWeek["Domingo"] as $days){
            if(isset($_GET['status']) && !empty($_GET['status'])){
                $query = $conn->prepare("SELECT COUNT(o.order_id) AS total FROM orders AS o $per_local_operator INNER JOIN sales AS s ON o.sale_id = s.sale_id WHERE o.order_date LIKE :order_date AND o.order_status = :status   AND o.user__id = :user__id $per_product");
                $query->execute(['order_date' => $days.'%', 'status' => $per_status, 'user__id' => $user__id]);
            }else{
                $query = $conn->prepare("SELECT COUNT(o.order_id) AS total FROM orders AS o $per_local_operator INNER JOIN sales AS s ON o.sale_id = s.sale_id WHERE o.order_date LIKE :order_date AND o.order_status = 0 AND o.user__id = :user__id $per_product");
    ...
    // var_dump($sun, $mon, $tue, $wed, $thu, $fri, $sat);      
        ...
        foreach($orders as $order){
            ...
            if($order['order_status'] == 3) {
               ...
               # Calculo de taxa financeira do cartão 
                ...
                // var_dump($get_operations_id->rowCount() > 0);
    ...
    $total_sales = $conn->prepare("SELECT COUNT(o.order_id) AS total FROM orders_all AS o INNER JOIN local_operations_orders loo ON loo.order_id = o.order_id");
    $total_sales->execute();
    ...
    foreach ($operations as $operation) {
        // var_dump($operation['operation_id']);
        $total_orders = $conn->prepare("SELECT COUNT(o.order_id) AS total FROM orders_all AS o INNER JOIN local_operations_orders loo ON loo.order_id = o.order_id WHERE loo.operation_id = :operation_id");
    ...
    */
    ...
    # 0 - Agendada 
    $get_sales_status0 = $conn->prepare('SELECT A.Q FROM (SELECT COUNT(order_id) AS Q, order_delivery_date, order_status FROM orders_zer AS o WHERE o.order_status = 0) as A');
    $get_sales_status0->execute();
    $sales0 = $get_sales_status0->fetch();

    # 1 - Reagendada
    $get_sales_status1 = $conn->prepare('SELECT A.Q FROM (SELECT COUNT(order_id) AS Q, order_delivery_date, order_status FROM orders_one AS o WHERE o.order_status = 1) as A');
    $get_sales_status1->execute();
    $sales1 = $get_sales_status1->fetch();

    # 2 - Atrasada
    $get_sales_status2 = $conn->prepare('SELECT A.Q FROM (SELECT COUNT(order_id) AS Q, order_delivery_date, order_status FROM orders_two AS o WHERE o.order_status = 2) as A');
    $get_sales_status2->execute();
    $sales2 = $get_sales_status2->fetch();


    # 3 - Completa
    $get_sales_status3 = $conn->prepare('SELECT A.Q FROM (SELECT COUNT(order_id) AS Q, order_delivery_date, order_status FROM orders_thr AS o WHERE o.order_status = 3) as A');
    $get_sales_status3->execute();
    $sales3 = $get_sales_status3->fetch();


    # 4 - Frustrada
    $get_sales_status4 = $conn->prepare('SELECT A.Q FROM (SELECT COUNT(order_id) AS Q, order_delivery_date, order_status FROM orders_fou AS o WHERE o.order_status = 4) as A');
    $get_sales_status4->execute();
    $sales4 = $get_sales_status4->fetch();

    # 5 - Cancelada
    $get_sales_status5 = $conn->prepare('SELECT A.Q FROM (SELECT COUNT(order_id) AS Q, order_delivery_date, order_status FROM orders_fiv AS o WHERE o.order_status = 5) as A');
    $get_sales_status5->execute();
    $sales5 = $get_sales_status5->fetch();


    # 6 - Reembolsada
    $get_sales_status6 = $conn->prepare('SELECT A.Q FROM (SELECT COUNT(order_id) AS Q, order_delivery_date, order_status FROM orders_six AS o WHERE o.order_status = 6) as A');
    $get_sales_status6->execute();
    $sales6 = $get_sales_status6->fetch();

    $haveDataFromGraphsSales = $sales0[0] + $sales1[0] + $sales2[0] + $sales3[0] + $sales4[0] + $sales5[0] + $sales6[0];
