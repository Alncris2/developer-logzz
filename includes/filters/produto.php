<?php
    ...
    $create_temp_table = $conn->prepare('CREATE TEMPORARY TABLE orders_all SELECT * FROM orders WHERE (user__id = :user__id AND product_id IN (:filter_by_product)) AND order_date BETWEEN :date_init AND :date_end AND platform IS null AND order_status = 3');
    $create_temp_table->bindValue(':user__id', $user__id, PDO::PARAM_INT);
    $create_temp_table->bindValue(':filter_by_product', $filter_by_product, PDO::PARAM_STR);
    ...
    $create_temp_table = $conn->prepare('CREATE TEMPORARY TABLE orders_zer SELECT * FROM orders WHERE (user__id = :user__id AND product_id IN (:filter_by_product)) AND (order_status = 0 AND order_date BETWEEN :date_init AND :date_end) AND platform IS null');
    $create_temp_table->execute(array('user__id' => $user__id, 'date_init' => $date_init, 'date_end' => $date_end, 'filter_by_product' => $filter_by_product));

    $create_temp_table = $conn->prepare('CREATE TEMPORARY TABLE orders_one SELECT * FROM orders WHERE (user__id = :user__id AND product_id IN (:filter_by_product)) AND (order_status = 1 AND order_date BETWEEN :date_init AND :date_end) AND platform IS null');
    $create_temp_table->execute(array('user__id' => $user__id, 'date_init' => $date_init, 'date_end' => $date_end, 'filter_by_product' => $filter_by_product));

    $create_temp_table = $conn->prepare('CREATE TEMPORARY TABLE orders_two SELECT * FROM orders WHERE (user__id = :user__id AND product_id IN (:filter_by_product)) AND (order_status = 2 AND order_date BETWEEN :date_init AND :date_end) AND platform IS null');
    $create_temp_table->execute(array('user__id' => $user__id, 'date_init' => $date_init, 'date_end' => $date_end, 'filter_by_product' => $filter_by_product));

    $create_temp_table = $conn->prepare('CREATE TEMPORARY TABLE orders_thr SELECT * FROM orders WHERE (user__id = :user__id AND product_id IN (:filter_by_product)) AND (order_status = 3 AND order_date BETWEEN :date_init AND :date_end) AND platform IS null');
    $create_temp_table->execute(array('user__id' => $user__id, 'date_init' => $date_init, 'date_end' => $date_end, 'filter_by_product' => $filter_by_product));

    $create_temp_table = $conn->prepare('CREATE TEMPORARY TABLE orders_fou SELECT * FROM orders WHERE (user__id = :user__id AND product_id IN (:filter_by_product)) AND (order_status = 4 AND order_date BETWEEN :date_init AND :date_end) AND platform IS null');
    $create_temp_table->execute(array('user__id' => $user__id, 'date_init' => $date_init, 'date_end' => $date_end, 'filter_by_product' => $filter_by_product));

    $create_temp_table = $conn->prepare('CREATE TEMPORARY TABLE orders_fiv SELECT * FROM orders WHERE (user__id = :user__id AND product_id IN (:filter_by_product)) AND (order_status = 5 AND order_date BETWEEN :date_init AND :date_end) AND platform IS null');
    $create_temp_table->execute(array('user__id' => $user__id, 'date_init' => $date_init, 'date_end' => $date_end, 'filter_by_product' => $filter_by_product));

    $create_temp_table = $conn->prepare('CREATE TEMPORARY TABLE orders_six SELECT * FROM orders WHERE (user__id = :user__id AND product_id IN (:filter_by_product)) AND (order_status = 6 AND order_date BETWEEN :date_init AND :date_end) AND platform IS null');
    $create_temp_table->execute(array('user__id' => $user__id, 'date_init' => $date_init, 'date_end' => $date_end, 'filter_by_product' => $filter_by_product));
