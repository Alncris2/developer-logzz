<?php
    ...
    $create_temp_table = $conn->prepare('CREATE TEMPORARY TABLE orders_all_date_filter SELECT * FROM orders WHERE (user__id = :user__id AND order_status = :filter_by_status) AND order_date BETWEEN :date_init AND :date_end AND platform IS null AND order_status = 3');
    $create_temp_table->bindValue(':user__id', $user__id, PDO::PARAM_INT);
    $create_temp_table->bindValue(':filter_by_status', $filter_by_status, PDO::PARAM_STR);
    ...
    $create_temp_table = $conn->prepare('CREATE TEMPORARY TABLE orders_all SELECT * FROM orders_all_date_filter WHERE product_id = 0 AND platform IS null');
    $create_temp_table->execute();

    foreach ($filter_by_product as $product) {

        $insert_into_temp_table = $conn->prepare('INSERT INTO orders_all (SELECT * FROM orders_all_date_filter WHERE product_id = :product)');
