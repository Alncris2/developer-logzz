<?php
    ...
    $create_temp_table = $conn->prepare('DROP TABLE IF EXISTS orders_all_date_filter');
    $create_temp_table->execute(array());
    ...
    $create_temp_table = $conn->prepare('CREATE TEMPORARY TABLE orders_all_date_filter SELECT * FROM orders WHERE platform IS null AND order_status = 3 AND user__id = :user__id AND order_date BETWEEN :date_init AND :date_end');
    $create_temp_table->bindValue(':user__id', $user__id, PDO::PARAM_INT);
    $create_temp_table->bindValue(':date_init', $date_init, PDO::PARAM_STR);
    $create_temp_table->bindValue(':date_end', $date_end, PDO::PARAM_STR);
    $create_temp_table->execute();

    $create_temp_table = $conn->prepare('CREATE TEMPORARY TABLE orders_all SELECT * FROM orders_all_date_filter WHERE product_id = 0 AND platform IS null  AND order_status = 3');
    $create_temp_table->execute();

    foreach ($filter_by_product as $product) {

        $insert_into_temp_table = $conn->prepare('INSERT INTO orders_all (SELECT * FROM orders_all_date_filter WHERE product_id = :product AND platform IS null AND order_status = 3)');
        $insert_into_temp_table->bindValue(':product', $product, PDO::PARAM_INT); 
        $insert_into_temp_table->execute();

    }

    $create_temp_table = $conn->prepare('CREATE TEMPORARY TABLE orders_zer SELECT * FROM orders_all WHERE user__id = :user__id AND order_status = 0 AND platform IS null');
    $create_temp_table->execute(array('user__id' => $user__id));

    $create_temp_table = $conn->prepare('CREATE TEMPORARY TABLE orders_one SELECT * FROM orders_all WHERE user__id = :user__id AND order_status = 1 AND platform IS null');
    $create_temp_table->execute(array('user__id' => $user__id));

    $create_temp_table = $conn->prepare('CREATE TEMPORARY TABLE orders_two SELECT * FROM orders_all WHERE user__id = :user__id AND order_status = 2 AND platform IS null');
    $create_temp_table->execute(array('user__id' => $user__id));

    $create_temp_table = $conn->prepare('CREATE TEMPORARY TABLE orders_thr SELECT * FROM orders_all WHERE user__id = :user__id AND order_status = 3 AND platform IS null');
    $create_temp_table->execute(array('user__id' => $user__id));

    $create_temp_table = $conn->prepare('CREATE TEMPORARY TABLE orders_fou SELECT * FROM orders_all WHERE user__id = :user__id AND order_status = 4 AND platform IS null');
    $create_temp_table->execute(array('user__id' => $user__id));

    $create_temp_table = $conn->prepare('CREATE TEMPORARY TABLE orders_fiv SELECT * FROM orders_all WHERE user__id = :user__id AND order_status = 5 AND platform IS null');
    $create_temp_table->execute(array('user__id' => $user__id));

    $create_temp_table = $conn->prepare('CREATE TEMPORARY TABLE orders_six SELECT * FROM orders_all WHERE user__id = :user__id AND order_status = 6 AND platform IS null');
    $create_temp_table->execute(array('user__id' => $user__id));
    ...
    $get_money_payment = $conn->prepare('SELECT A.quant FROM (SELECT COUNT(order_payment_method) AS quant, order_delivery_date, order_payment_method AS method FROM orders_thr WHERE order_payment_method = "money" AND platform IS null ) as A');
    ...
    $get_credit_payment = $conn->prepare('SELECT A.quant FROM (SELECT COUNT(order_payment_method) AS quant, order_delivery_date, order_payment_method AS method FROM orders_thr WHERE order_payment_method = "credit" AND platform IS null) as A');
    ...
    $get_debit_payment = $conn->prepare('SELECT A.quant FROM (SELECT COUNT(order_payment_method) AS quant, order_delivery_date, order_payment_method AS method FROM orders_thr WHERE order_payment_method = "debit" AND platform IS null) as A');
    ...
    $get_pix_payment = $conn->prepare('SELECT A.quant FROM (SELECT COUNT(order_payment_method) AS quant, order_delivery_date, order_payment_method AS method FROM orders_thr WHERE order_payment_method = "pix" AND platform IS null) as A');
    ...
    # Cálculo Faturamento
    $faturamento = $conn->prepare('SELECT A.total FROM (SELECT SUM(order_final_price) AS total, order_delivery_date FROM orders_all WHERE platform IS null) as A');
    $faturamento->execute();
    ...
    $vendas = $conn->prepare('SELECT COUNT(*), order_delivery_date FROM orders_all WHERE platform IS null');
    $vendas->execute();
    ...
    $clientes = $conn->prepare('SELECT * FROM orders_all WHERE platform IS null GROUP BY order_number');
    $clientes->execute();
    $clientes = $clientes->rowCount();

    $reembolsos = $conn->prepare('SELECT * FROM orders_all WHERE order_status = 6 AND platform IS null GROUP BY order_number');
    $reembolsos->execute();
