<?php
    ...
    $create_temp_table = $conn->prepare('CREATE TEMPORARY TABLE orders_nine SELECT * FROM orders WHERE user__id = :user__id AND (order_status = 9 AND order_delivery_date BETWEEN :date_init AND :date_end) AND platform IS null');
    $create_temp_table->execute(array('user__id' => $user__id, 'date_init' => $date_init, 'date_end' => $date_end));

    $create_temp_table = $conn->prepare('CREATE TEMPORARY TABLE orders_ten SELECT * FROM orders WHERE user__id = :user__id AND (order_status = 10 AND order_delivery_date BETWEEN :date_init AND :date_end) AND platform IS null');
    $create_temp_table->execute(array('user__id' => $user__id, 'date_init' => $date_init, 'date_end' => $date_end));

    $create_temp_table = $conn->prepare('CREATE TEMPORARY TABLE orders_eleven SELECT * FROM orders WHERE user__id = :user__id AND (order_status = 11 AND order_delivery_date BETWEEN :date_init AND :date_end) AND platform IS null');
    $create_temp_table->execute(array('user__id' => $user__id, 'date_init' => $date_init, 'date_end' => $date_end));

    $create_temp_table = $conn->prepare('CREATE TEMPORARY TABLE orders_twelve SELECT * FROM orders WHERE user__id = :user__id AND (order_status = 12 AND order_delivery_date BETWEEN :date_init AND :date_end) AND platform IS null');
    $create_temp_table->execute(array('user__id' => $user__id, 'date_init' => $date_init, 'date_end' => $date_end));
    ...
    */
    ...
    # 9 - Reembolsada
    $get_sales_status9 = $conn->prepare('SELECT A.Q FROM (SELECT COUNT(order_id) AS Q, order_delivery_date, order_status FROM orders_nine) as A');
    $get_sales_status9->execute(); 
    $sales9 = $get_sales_status9->fetch();


    # 10 - Confirmado
    $get_sales_status10 = $conn->prepare('SELECT A.Q FROM (SELECT COUNT(order_id) AS Q, order_delivery_date, order_status FROM orders_ten) as A');
    $get_sales_status10->execute();
    $sales10 = $get_sales_status10->fetch();


    # 11 - Em Aberto
    $get_sales_status11 = $conn->prepare('SELECT A.Q FROM (SELECT COUNT(order_id) AS Q, order_delivery_date, order_status FROM orders_eleven) as A');
    $get_sales_status11->execute();
    $sales11 = $get_sales_status11->fetch();


    # 12 - Indisponível
    $get_sales_status12 = $conn->prepare('SELECT A.Q FROM (SELECT COUNT(order_id) AS Q, order_delivery_date, order_status FROM orders_twelve) as A');
