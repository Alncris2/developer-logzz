...
function sendWebhookNovosUsuarios($data)
{
    ...
    $get_integration = $conn->prepare('SELECT * FROM atendezap_integration WHERE az_level LIKE "owner" AND az_name LIKE "new_user" AND az_active = 1 LIMIT 1');
...
function updateUserOnStatus2($order_id)
{
    ...
    $sqlOrdersSendWhats = "SELECT order_id, 
    client_number, order_number, order_status_description, client_name, product_name, CASE WHEN orders.platform <> null OR orders.platform != '' THEN orders.order_quantity ELSE s.sale_quantity END AS quantity, order_status FROM orders INNER JOIN sales s ON orders.sale_id = s.sale_id WHERE order_id = :order_id";
