...
<link rel="stylesheet" type="text/css" href="https://cdn.datatables.net/buttons/2.0.1/css/buttons.dataTables.min.css"/>
	...
	<script src="<?php echo SERVER_URI; ?>/js/picker.date.min.js" type="text/javascript"></script>
	...
	<script src="<?php echo SERVER_URI; ?>/js/main.js?v=2" type="text/javascript"></script>
...
<?php
	...
	*/
    ...
    if (isset($orders_page)) {
        echo '	
	<script src="' . SERVER_URI . '/js/orders.js?v=3" type="text/javascript"></script>';
    }

		    # Scripts específicos das páginas de pedidos.
	if (isset($orders_operator_page)) {
		echo '<script src="' . SERVER_URI . '/js/orders.js" type="text/javascript"></script>';
		echo '<script src="' . SERVER_URI . '/js/orders_operator.js?v=1" type="text/javascript"></script>';
	}
    ...
    if (isset($sale_page)) {
        echo '	
        <script src="' . SERVER_URI . '/js/sale.js?v=3" type="text/javascript"></script>';
    }
	...
	if (isset($subscriber_page)){
		echo '	
	<script src="' . SERVER_URI . '/js/subscriber.js?v=3" type="text/javascript"></script>';
	}

    	# Scripts específicos da página de integrar usuario com a tiny.
	if (isset($users_integrations)){
		echo '
		<script src="' . SERVER_URI . '/js/integration_users.js?v=1" type="text/javascript"></script>';
	}
    ...
    if (isset($inventories_page)) {
        echo '	
        <script src="' . SERVER_URI . '/js/inventorie.js?v=5" type="text/javascript"></script>';
    }
	...
	if (isset($subscriber_plan_details)){
		if (!($subscriber_plan_details['gateway'] > 0)){$subscriber_plan_details['gateway'] = 0;}
		if (!($subscriber_plan_details['plano'] > 0)){$subscriber_plan_details['plano'] = 0;}
		if (!($subscriber_plan_details['taxa'] > 0)){$subscriber_plan_details['taxa'] = 0;}
		if (!($subscriber_plan_details['prazo'] > 0)){$subscriber_plan_details['prazo'] = 0;}
		echo '  
	<script type="text/javascript">
		$(document).ready(function () {var select_taxa_gateway_assinante = $("#select-taxa-gateway-assinante").select();select_taxa_gateway_assinante.val(' . $subscriber_plan_details['gateway'] . ').trigger("change"); var select_plano_assinante = $("#select-plano-assinante").select();select_plano_assinante.val(' . $subscriber_plan_details['plano'] . ').trigger("change");var value = $("#select-plano-assinante").val();$("#text-plano-assinante").val(value);var select_taxa_assinante = $("#select-taxa-assinante").select();select_taxa_assinante.val(' . $subscriber_plan_details['taxa'] . ').trigger("change");var value = $("#select-taxa-assinante").val();$("#text-taxa-assinante").val(value);var select_prazo_assinante = $("#select-prazo-assinante").select();select_prazo_assinante.val(' . $subscriber_plan_details['prazo'] . ').trigger("change");var value = $("#select-prazo-assinante").val();$("#text-prazo-assinante").val(value);});</script>';
	}
...
?>
	<script>
				...
				new Chart(salesStatusChart, {
					...
					data: {
						datasets: [{
							data: [<?php echo $dashboard_charts['agendada'];  ?>, <?php echo $dashboard_charts['reagendada'];  ?>, <?php echo $dashboard_charts['completa'];  ?>, <?php echo $dashboard_charts['atrasada'];  ?>, <?php echo $dashboard_charts['frustrada'];  ?>, <?php echo $dashboard_charts['cancelada'];  ?>],
							borderWidth: 0,
							backgroundColor: 		["#2fde91", "#4fdf1f", "#0b352b", "#f7e39c", "#fc8567","#db323d", "#aaaba4"],
							hoverBackgroundColor: 	["#2fde91", "#4fdf1f", "#0b352b", "#f7e39c", "#fc8567","#db323d", "#aaaba4"]

						}],
						labels: 					["Agendada", "Reagendada", "Completa", "Atrasada", "Frustrada", "Cancelada", "Reembolsada"]
					},
					options: {
						responsive: true,
						legend: false,
