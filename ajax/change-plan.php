		...
		$user_phone = "+55" . (str_replace($chars, "", $current_plan_details['user_phone']));
			...
			// try {
				...
				do {
					...
					$tries = $tries + 1;
					sleep(1);
				} while ($transaction_status == 'processing' && $tries < 15);
				...
				if ($transaction_status == 'paid'){
					...
					$update_user_plan = $conn->prepare('UPDATE subscriptions SET subscription_current_plan = :subscription_current_plan, user_plan = :user_plan, user_plan_tax = :user_plan_tax, user_external_gateway_tax = :user_external_gateway_tax, user_plan_shipping_tax = :user_plan_shipping_tax, user_payment_term = :user_payment_term, custom_conditions = :custom_conditions, subscription_start = :subscription_start, subscription_renewal = :subscription_renewal, plan_price = :plan_price WHERE subscription_id = :subscription_id');
					$update_user_plan->execute(array('subscription_current_plan' => $new_plan_id, 'user_plan' => $new_plan_id, 'user_plan_tax' => $user_plan_tax, 'user_external_gateway_tax' => $user_external_gateway_tax, 'user_plan_shipping_tax' => $user_plan_shipping_tax, 'user_payment_term' => $user_payment_term, 'custom_conditions' => $custom_conditions, 'subscription_start' => $subscription_start, 'subscription_renewal' => $subscription_renewal, 'plan_price' => $new_plan_price, 'subscription_id' => $subscription_id));
					...
					# Cria um histórico da cobrança
					$create_billing_request = $conn->prepare('INSERT INTO billings (billing_id, user__id, billing_value, billing_bank_account, billing_type, billing_request, billing_released) VALUES (:billing_id, :user__id, :billing_value, :billing_bank_account, :billing_type, :billing_request, :billing_released)');
					$create_billing_request->execute(array('billing_id' => '0', 'user__id' => $user__id, 'billing_value' => ($amount / 100), 'billing_bank_account' => '0', 'billing_type' => 'PLAN_UPGRADE', 'billing_request' => $subscription_start, 'billing_released' => $subscription_start));

					# Exclui mudanças de plano agendadas
					$delete_scheduled_changes = $conn->prepare('DELETE FROM subscriptions_meta WHERE meta_key = "plan_change_scheduled" AND subscription_id = :user__id');
					$delete_scheduled_changes->execute(array('user__id' => $user__id));
