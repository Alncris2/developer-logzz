...
if ($_POST['action'] == 'new-subscriber'){
	...
	$stmt = $conn->prepare('UPDATE users SET full_name = :full_name, user_plan = :user_plan, user_plan_tax = :user_plan_tax, user_plan_shipping_tax = :user_plan_shipping_tax, user_payment_term = :user_payment_term WHERE user_code = :user_code');

	try {

		$stmt->execute(array('full_name' => $full_name, 'user_plan' => $user_plan, 'user_plan_tax' => $user_plan_tax, 'user_plan_shipping_tax' => $user_plan_shipping_tax, 'user_payment_term' => $user_payment_term, 'user_code' => $user_code));
