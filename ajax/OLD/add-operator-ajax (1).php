...
if ($_POST['action'] == 'new-operator'){
  ...
  if (!($verify_unique_user_code->rowCount() == 0)) {
	...
	$stmt = $conn->prepare('INSERT INTO users (user__id, full_name, user_password, email, user_avatar, active, created_at, updated_at, user_code) VALUES (:user__id, :full_name, :user_password, :email, :user_avatar, :active, :created_at, :updated_at, :user_code)');
  ...
  $get_last_id = $conn->prepare('SELECT user__id FROM users ORDER BY user__id DESC LIMIT 1');
	...
	try {
		...
		$stmt->execute(array('user__id' => $user__id, 'full_name' => $nome_operador, 'user_password' => $senha, 'email' => $email, 'user_avatar' => "profile.jpg", 'active' => 1, 'created_at' => $created_at, 'updated_at' => $updated_at, 'user_code' => $user_code));
    $get_last_id->execute();
