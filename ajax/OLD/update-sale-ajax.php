...
$charge_shipping = 0;
$sale_url_redirect_post = 0;
$sale_url_redirect_or = 0;
...
if (isset($sale_custom_commission)) {
    ...
    if ($get_sale_custom_commision->rowCount() == 0) {
		...
		$stmt = $conn->prepare('UPDATE sales SET sale_name = :sale_name, sale_quantity  = :sale_quantity, sale_price = :sale_price, sale_url = :sale_url, sale_fb_pixel = :sale_fb_pixel, meta_pixel_facebook_api = :meta_pixel_facebook_api, one_click_url = :one_click_url, url_upsell = :url_upsell, sale_cost = :sale_cost, sale_tax = :sale_tax, charge_shipping = :charge_shipping, sale_url_redirect_post = :sale_url_redirect_post, sale_url_redirect_or = :sale_url_redirect_or  WHERE sale_id = :sale_id');
		
		try {
			$stmt->execute(array('sale_name' => $sale_name, 'sale_quantity' => $sale_quantity, 'sale_price' => $sale_price, 'sale_url' => $sale_url, 'sale_fb_pixel' => $fb_pixel, 'meta_pixel_facebook_api' => $facebook_api, 'sale_id' => $sale_id, 'one_click_url' => $one_click_url, 'url_upsell' => $url_upsell, 'sale_cost' => $sale_cost, 'sale_tax' => $sale_tax, 'charge_shipping' => $charge_shipping, 'sale_url_redirect_post' => $sale_url_redirect_post, 'sale_url_redirect_or' => $sale_url_redirect_or));
