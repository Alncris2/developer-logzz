    ...
    $inventory_last_ship_quant = addslashes($_GET['quant']);
        ...
        switch ($shipment_status) {
            ...
            case 2:
                ...
                $verify_current_status = $conn->prepare('SELECT inventory_id, inventory_quantity FROM inventories WHERE inventory_meta = :inventory_meta AND ship_locale = :ship_locale');
                $verify_current_status->execute(array('inventory_meta' => $inventory_meta, 'ship_locale' => $type_locale));
                ...
                if ($verify_current_status->rowCount() > 0) {
                    ...
                    $stmt = $conn->prepare('UPDATE inventories SET inventory_quantity = :inventory_quantity, inventory_last_ship = :inventory_last_ship, inventory_last_ship_quant = :inventory_last_ship_quant  WHERE inventory_meta = :inventory_meta AND ship_locale = :ship_locale');

                    $stmt->execute(array('inventory_quantity' => $new_value, 'inventory_last_ship' => $inventory_last_ship, 'inventory_last_ship_quant' => $inventory_last_ship_quant, 'inventory_meta' => $inventory_meta, 'ship_locale' => $type_locale));
          ...
          $stmt = $conn->prepare('UPDATE shipments SET ship_status = :shipment_status WHERE ship_id = :shipment_id AND type_shipments = :type_shipments');
          $stmt->execute(array('shipment_status' => $shipment_status, 'shipment_id' => $shipment_id, 'type_shipments' => $type_locale));
