...
try {
    ...
    $get_membership_meta_pixel = $conn->prepare('SELECT meta_id FROM memberships_meta WHERE (membership_hotcode = :membership_hotcode AND sale_id = :sale_id) AND meta_key = "fb_pixel"');
    $get_membership_meta_pixel->execute(array('sale_id' => $sale_id, 'membership_hotcode' => $membership_hotcode));
    if ($get_membership_meta_pixel->rowCount() == 0) {
        $set_membership_meta_pixel = $conn->prepare('INSERT INTO memberships_meta (meta_id, membership_hotcode, sale_id, meta_key, meta_value) VALUES (:meta_id, :membership_hotcode, :sale_id, "fb_pixel", :meta_value)');
        $set_membership_meta_pixel->execute(array('meta_id' => 0, 'membership_hotcode' => $membership_hotcode, 'sale_id' => $sale_id, 'meta_value' => $facebook_pixel));
    } else {
    ...
    $get_membership_meta_fb_capi = $conn->prepare('SELECT meta_id FROM memberships_meta WHERE (membership_hotcode = :membership_hotcode AND sale_id = :sale_id) AND meta_key = "fb_capi"');
    $get_membership_meta_fb_capi->execute(array('sale_id' => $sale_id, 'membership_hotcode' => $membership_hotcode));
    if ($get_membership_meta_fb_capi->rowCount() == 0) {
        $set_membership_meta_fb_capi = $conn->prepare('INSERT INTO memberships_meta (meta_id, membership_hotcode, sale_id, meta_key, meta_value) VALUES (:meta_id, :membership_hotcode, :sale_id, "fb_capi", :meta_value)');
        $set_membership_meta_fb_capi->execute(array('meta_id' => 0, 'membership_hotcode' => $membership_hotcode, 'sale_id' => $sale_id, 'meta_value' => $facebook_api));
    } else {
    ...
    $get_membership_meta_google_ua = $conn->prepare('SELECT meta_id FROM memberships_meta WHERE (membership_hotcode = :membership_hotcode AND sale_id = :sale_id) AND meta_key = "google_ua"');
    $get_membership_meta_google_ua->execute(array('sale_id' => $sale_id, 'membership_hotcode' => $membership_hotcode));
    if ($get_membership_meta_google_ua->rowCount() == 0) {
        $set_membership_meta_google_ua = $conn->prepare('INSERT INTO memberships_meta (meta_id, membership_hotcode, sale_id, meta_key, meta_value) VALUES (:meta_id, :membership_hotcode, :sale_id, "google_ua", :meta_value)');
        $set_membership_meta_google_ua->execute(array('meta_id' => 0, 'membership_hotcode' => $membership_hotcode, 'sale_id' => $sale_id, 'meta_value' => $google_ua));
    } else {
    ...
    $get_membership_meta_google_aw = $conn->prepare('SELECT meta_id FROM memberships_meta WHERE (membership_hotcode = :membership_hotcode AND sale_id = :sale_id) AND meta_key = "google_aw"');
    $get_membership_meta_google_aw->execute(array('sale_id' => $sale_id, 'membership_hotcode' => $membership_hotcode));
    if ($get_membership_meta_google_aw->rowCount() == 0) {
        $set_membership_meta_google_aw = $conn->prepare('INSERT INTO memberships_meta (meta_id, membership_hotcode, sale_id, meta_key, meta_value) VALUES (:meta_id, :membership_hotcode, :sale_id, "google_aw", :meta_value)');
        $set_membership_meta_google_aw->execute(array('meta_id' => 0, 'membership_hotcode' => $membership_hotcode, 'sale_id' => $sale_id, 'meta_value' => $google_aw));
    } else {
    ...
    $get_membership_meta_tiktok_pixel = $conn->prepare('SELECT meta_id FROM memberships_meta WHERE (membership_hotcode = :membership_hotcode AND sale_id = :sale_id) AND meta_key = "tiktok_pixel"');
    $get_membership_meta_tiktok_pixel->execute(array('sale_id' => $sale_id, 'membership_hotcode' => $membership_hotcode));
    if ($get_membership_meta_tiktok_pixel->rowCount() == 0) {
        $set_membership_meta_tiktok_pixel = $conn->prepare('INSERT INTO memberships_meta (meta_id, membership_hotcode, sale_id, meta_key, meta_value) VALUES (:meta_id, :membership_hotcode, :sale_id, "tiktok_pixel", :meta_value)');
        $set_membership_meta_tiktok_pixel->execute(array('meta_id' => 0, 'membership_hotcode' => $membership_hotcode, 'sale_id' => $sale_id, 'meta_value' => $tiktok_pixel));
    } else {
    ...
    $feedback = array('title' => "Erro Interno", 'type' => 'warning', 'msg' => 'Não foi possível atualizar os dados');
    echo json_encode($feedback);
    exit;
