...
// DELETE FROM `transactions_meta` WHERE `meta_key` IN ('anticipated_value', 'anticipation_balance', 'cashed_out_balance', 'commission_balance', 'in_review_balance');
// UPDATE orders o SET o.order_commission_released = 0, o.order_anticipation_released = 0; 
...
// echo "Buscando comissões não liberadas p/ Valor TOTAL com a data igual ou inferior a ontem.... <br>";
// $get_commissions = $conn->prepare('SELECT user__id, SUM(order_liquid_value) AS total_value FROM orders WHERE order_status = 3 GROUP BY user__id');
...
//         $new_total_value = round(($billing_value + $old_total_value), 2);
...
//     echo "Criada ANTECIPAÇÃO DISPONÍVEL de <b>" . $new_total_value ." (". $old_total_value .")</b> para o usuário <b>". $user__id . "</b><br>";
...
// $get_commissions = $conn->prepare('SELECT user__id, SUM(order_liquid_value) AS total_value FROM orders WHERE order_status = 3 AND (order_commission_date <= :_date AND order_commission_released = 0) GROUP BY user__id');
...
//         $new_total_value = round(($old_total_value + $billing_value), 2);
...
//     echo "Atualizado VALOR TOTAL ARRECADADO de <b>" . $new_total_value ." (". $old_total_value .")</b> para o usuário <b>". $user__id . "</b><br>"; 
// } 
