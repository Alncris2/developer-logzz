    ...
    <div class="row my-5" style="justify-content: space-between;">
        ...
        <div class="col-12">
            ...
            <div class="card" style="margin:6px 0">
                ...
                <?php 
                $get_notifications = $conn->prepare('SELECT * FROM `notifications` WHERE (`user__id` = :user__id OR `user__id` IS NULL) AND `notification_open` = 0 ');
                $get_notifications->execute(array('user__id' => $_SESSION['UserID']));
                $my_notifications =  $get_notifications->fetchALL();
                if($my_notifications){ ?> 
                    <ul class="list-group list-group-flush mb-4">
                        <?php foreach($my_notifications as $my_n){ ?> 
                            <li class="list-group-item">
                                <div class="read-notification" data-id="<?= $my_n['id_notification'] ?>" data-url="<?= $my_n['notification_link'] ?>"> 
                                    <div class="row w-100 ml-0">  
                                        <div class="align-items-center d-flex justify-content-centera col-2">
                                            <i class="<?= $my_n['notification_icon'] ?> fa-2x text-primary"></i>
                                        </div>
                                        <div class="align-items-center d-flex justify-content-between col-10"> 
                                            <span><?= $my_n['notification_context'] ?></span>
                                            <span><?= date_format(date_create($my_n['created_at']), 'd/m \<\b\r\> H:i') ?></span>
                                        </div>
                                    </div>
                                </div>      
                            </li>
