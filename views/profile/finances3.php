        ...
        <div class="card accordion__item">
            ...
            <div id="historic-transaction-collapse" class="card-bodyaccordion__body" data-parent="#finances-accordion">
                <div class="accordion__body--text">
                    <form id="filter-historic-transaction">
                        <div class="w-100 row">
                            <div class="form-group col-6">
                                ...
                                <select class="segment-select" id="select-filter-days" name="filter-days">
                                ...
                                <div class="btn-group ">
                                    <button id="filter-data-custom" class="d-none dropdown-toggle" type="button" data-toggle="dropdown" aria-expanded="false"></button>
                                    <div class="dropdown-menu dropdown-menu-lg-right mx-2">
                                        <div class="input-group form-control mb-2">
                                            <input name="data-inicio" value="" placeholder="Do dia ..." class="datepicker-default picker__input" id="data-inicio" readonly="" aria-haspopup="true" aria-expanded="false" aria-readonly="false" aria-owns="datepicker_root">
                                        </div>

                                        <div class="input-group form-control mb-2">
                                            <input name="data-final" value="" placeholder=".. ao dia" class="datepicker-default picker__input" id="data-final" readonly="" aria-haspopup="true" aria-expanded="false" aria-readonly="false" aria-owns="datepicker_root">
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div id="historic-transaction-accordion" class="accordion accordion-with-icon col-md-12">
                        <?php
                        ...
                        for ($aux = 0; $aux <= $ref_days; $aux++) {
                            ...
                            $get_balance_to_day = $conn->prepare("SELECT sum(value_liquid) from transactions where date_start <= :date_start AND user_id = :user_id AND (CASE WHEN type = 7 THEN date_end < now() ELSE date_end IS NOT NULL END)  ORDER BY date_start DESC");
                            ...
                            $get_transactions_days = $conn->prepare("SELECT user_id, date(date_format(date_start, '%Y-%m-%d')) AS transaction_day, sum(value_liquid) AS transaction_day_value, count(*) AS transaction_day_quant FROM transactions a WHERE user_id = :user_id AND date_start BETWEEN '$date_start 00:00:00' AND '$date_start 23:59:50' AND (date_end < now() OR date_end IS NULL) GROUP BY transaction_day ORDER BY `date_start` DESC");
                            ...
                            if ($get_transactions_days->rowCount() > 0) {
                                while ($transactions_days = $get_transactions_days->fetch()) { ?>
                                    <div class="card accordion__item">
                                        <div class="card-header" data-toggle="collapse" data-target="#historic-collapse-<?php echo date('Y-m-d', strtotime($transactions_days['transaction_day'])) ?>" aria-expanded="false">
                                            <div class="col-6 text-left d-flex">
                                                <h6><?php echo date_format(date_create($transactions_days['transaction_day']), 'd/m/y') ?>&nbsp; </h6>
                                                <small> &nbsp;<?= $transactions_days['transaction_day_quant'] . ($transactions_days['transaction_day_quant'] == 1 ? ' registro' : ' registros') ?></small>
                                            </div>
                                            <div class="col-6 text-right">
                                                Saldo&nbsp;&nbsp; <?= '<b> R$ ' . number_format($balance_day, 2, ',', '.') . " </b><small>(R$ " . number_format($transactions_days['transaction_day_value'], 2, ',', '.') . ")</small>"  ?></div>
                                            <a aria-expanded="false"><i style="color: #777777" class="fas fa-angle-down"></i></a>
                                        </div>
                                        <div id="historic-collapse-<?php echo date('Y-m-d', strtotime($transactions_days['transaction_day'])) ?>" class="card-bodyaccordion__body collapse" data-parent="#historic-transaction-accordion">
                                                    ...
                                                    while ($transactions_list = $get_transactions_list->fetch()) {
                                                        ...
                                                        <tr>
                                                            <td class="fs-12 text-center" data-order="<?php echo date_format(date_create($transactions_list['date_start']), 'YmdHis'); ?>">
                                                                <?php echo date_format(date_create($transactions_list['date_start']), 'd/m/y \<\b\r\> H:i'); ?>
                                                            </td>
                                                            ...
                                                            <td style="text-align: center; color: <?php echo $color; ?>; font-weight: bold;"><?php echo $signal . " R$ " . number_format($transactions_list['value_brute'], 2, ",", "."); ?></td>
                                                            ...
                                                            <td style="text-align: center; color: <?php echo $color; ?>; font-weight: bold;">
                                                                <?php
                                                                    if ($transactions_list['type'] == 'Antecipação') {
                                                                        echo "+ R$ " . number_format(($transactions_list['value_brute'] - $transactions_list['tax_value']), 2, ",", ".");
                                                                    }  else {
                                                                        echo " R$ " . number_format($transactions_list['value_liquid'], 2, ",", ".");
                                                                    } ?>
                                                            </td>
                                                            <td>
                                                                <?php if ($transactions_list['status'] == 'Cancelado') {
                                                                    echo '<span class="badge badge-sm d-block m-auto light badge-danger"><i class="far fa-check-circle"></i> Cancelado</span>';
                                                            ...
                                                            <td style="text-align: center;">
                                                                <?php if ($transactions_list['orders_antecipation']) { 
                                                                    if ($transactions_list['orders_antecipation']) {
                                                                        echo '<a  class="" title="Ver comprovante de trasnferência" data-orders-number="'. $transactions_list['orders_antecipation'] .'" data-cod-transaction="'. $transactions_list["transaction_code"] .'">
                                                                            <i class="fa fa-list"></i>
                                                                        </a>';
                                                                    } else {
                                                                        echo '<a title="Não temos informaçãoes dos pedidos antecipados :(">
                                                                        <i class="fa fa-eye-slash"></i>
                                                                        </a>';
                                                                    }

                                                                } else { ?>
                                                                    <a title="Ver comprovante de trasnferência" <?= $transactions_list['bank_proof'] == NULL ? "" :  "href='" . SERVER_URI . "/uploads/saques/comprovantes/" . $transactions_list['bank_proof'] . "' target='_blank'" ?>>
                                                                        <i class="fa fa-eye<?= $transactions_list['bank_proof'] == NULL ? '-slash' : '' ?>"></i>
                                                                    </a>
                                                                <?php } ?>
                                                            </td>
                                                        </tr>
                ...
                <div class="accordion__body--text">
                    <form id="filter-future-releases">
                        <div class="w-100 row">
                            ...
                            <div class="form-group col-4 my-2 me-2">
                                <label>Comissão a Receber:</label>
                                <p><strong>R$ 2.000,20</strong></p>
                            </div>
                            <div class="form-group col-2">
                                <label>&nbsp;</label>
                                <div class="" role="group">
                    ...
                    <div id="future-releases-accordion" class="accordion accordion-with-icon col-md-12">
                        ...
                        $get_transactions_days = $conn->prepare("SELECT date(date_format(date_start, '%Y-%m-%d')) AS transaction_day, sum(value_liquid) AS transaction_day_value, count(*) AS transaction_day_quant, (SELECT sum(value_liquid) from transactions b where b.date_start <= a.date_start AND b.user_id = a.user_id AND b.date_end > now() ORDER BY b.date_end ASC) as current_sum FROM transactions a WHERE user_id = :user_id AND date_start > :date_start AND date_end > now() GROUP BY transaction_day ORDER BY `date_start` ASC");
                        $get_transactions_days->execute(array('user_id' => $user__id, 'date_start' => $date_start));
                                    ...
                                    <div id="future-releases-<?php echo date('Y-m-d', strtotime($transactions_days['transaction_day'])) ?>" class="card-bodyaccordion__body collapse" data-parent="#historic-transaction-accordion">
                                        <table class="table movement-history">
                                            ...
                                            <tbody>
                                                ...
                                                $get_transactions_list = $conn->prepare('SELECT t.*, transaction_type_name as type, transaction_status_name as status FROM transactions t INNER JOIN transactions_type ON transaction_type_id = type INNER JOIN transaction_status ON transaction_status_id = status WHERE user_id = :user__id AND date_start between :date_filter_start AND :date_filter_end AND date_end > now() ORDER BY date_start ASC');
                                                $get_transactions_list->execute(array('user__id' => $user__id, 'date_filter_start' => $date_filter_start, 'date_filter_end' => $date_filter_end));
                                                while ($transactions_list = $get_transactions_list->fetch()) {
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td class="fs-12 text-center" data-order="<?php echo date_format(date_create($transactions_list['date_start']), 'YmdHis'); ?>">
                                                            <?php echo date_format(date_create($transactions_list['date_start']), 'd/m/y \<\b\r\> H:i'); ?></td>
                                                        <!-- <td style="text-align: center;">
                                                            <?php
                                                            if ($transactions_list['type'] == 'Pedido') {
                                                                echo "Entrada";
                                                        ...
                                                        <td class="fs-12 text-center" data-order="<?php echo date_format(date_create($transactions_list['date_end']), 'YmdHis'); ?>">
                                                            <?php echo date_format(date_create($transactions_list['date_end']), 'd/m/y \<\b\r\> H:i'); ?>
                                                        </td>
                                                        <td style="text-align: center;">
            ...
            <div class="card-body">
                ...
                <div id="orders-simulation" class="d-none mb-2 text-center">
                    <form id="anticipation-request-new">
                        ...
                        <button class="btn btn-success btn-lg btn-block" type="submit" id="anticipation-request-btn">
                ...
                <button class="btn btn-success btn-lg btn-block" id="anticipation-simulation-btn">
                    <i class="fas fa-hand-holding-usd"></i><span id="anticipation-simulation-text">Antecipar saldo</span>
                </button>
            </div>
        </div>
    </div>
    ...
    $(document).ready(function() {
        ...
        document.querySelector("#valor-antecipacao").addEventListener("input", () => {
            ...
            if (math.compare(ValueSimulationAnticipation, TotalAmountAmount) == 0) {
                ...
                $('#anticipation-simulation-text').html('Antecipar saldo');
                $('#text-value-available-antecipate').html('Disponível p/ Antecipação: <strong>' + dollarUS.format(TotalAmountAmount) + '</strong>').removeAttr('style');
                return;
            }
            if (math.compare(ValueSimulationAnticipation, TotalAmountAmount) > 0) {
